/*
 * $Header: /home/barad-dur/vision/forsyth/schenney/sced-0.94/c/RCS/View.c,v 1.0 1997/05/06 20:30:22 schenney Exp $
 *
 * $Log: View.c,v $
...
#define Set(b) (prop->request_mode & b)
    ...
    if ( Set(CWWidth) )
    {
        if ( Set(CWHeight) )
        {
            if ( prop->width == pref->width && prop->height == pref->height )
                return XtGeometryYes;
        }
        else if ( prop->width == pref->width )
            return XtGeometryYes;
    }
    else if ( Set(CWHeight) )
    {
        if ( prop->height == pref->height )
            return XtGeometryYes;
    }
    else
        return XtGeometryYes;
