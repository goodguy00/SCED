/*
 * $Header: /home/barad-dur/vision/forsyth/schenney/sced-0.94/c/RCS/bounds.c,v 1.0 1997/05/06 20:30:22 schenney Exp $
 *
 * $Log: bounds.c,v $
...
Cuboid
Calculate_Bounds_f(Vector *verts, int num_verts)
{
    Cuboid  res;
    int     i;
        ...
        if ( current_inst->the_instance->o_flags & ObjVisible )
            current_inst->the_instance->o_proj_extent =
                Calculate_Projection_Extents(
                    ...
                    current_inst->the_instance->o_num_real);
...
Calculate_Object_Bounds(ObjectInstancePtr obj)
{
    ...
    {
        case csg_obj:
            bound = ((CSGNodePtr)obj->o_parent->b_struct)->csg_bound;
            break;

        case aggregate_obj:
            bound = ((AggregatePtr)obj->o_parent->b_struct)->agg_bound;
            break;

        case wireframe_obj:
            bound = Calculate_Bounds(obj->o_parent->b_major_wires[0]->vertices,
                            obj->o_parent->b_major_wires[0]->num_real_verts);
            break;

        case torus_obj:
            VNew(((TorusPtr)obj->o_hook)->major_radius + 1.0,
                 ((TorusPtr)obj->o_hook)->major_radius + 1.0, 1, bound.max);
            VNew(-((TorusPtr)obj->o_hook)->major_radius - 1.0,
                 -((TorusPtr)obj->o_hook)->major_radius - 1.0, -1, bound.min);
            break;

        case triangle_obj:
        case bezier_obj:
            bound = Calculate_Bounds(control_part(obj)->control_verts,
                                     control_part(obj)->num_control_verts);
            break;
