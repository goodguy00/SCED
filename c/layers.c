/*
 * $Header: /home/barad-dur/vision/forsyth/schenney/sced-0.94/c/RCS/layers.c,v 1.0 1997/05/06 20:30:22 schenney Exp $
 *
 * $Log: layers.c,v $
...
Layer_Check_Instance_Visibility(LayerPtr layer, ObjectInstancePtr inst)
{
    ...
    if ( ! layer->state )
    {
        ...
        inst->o_flags |= ObjVisible;
}

static void
Layer_Add_Selected_Instances(int layer_num)
{
    LayerPtr        layer = (LayerPtr)Hash_Get_Value(layer_hash,
...
{
    LayerPtr    layer = (LayerPtr)Hash_Delete(layer_hash,(unsigned long)victim);
    int         index;
    Widget      temp;
    int         i;
...
{
    LayerPtr    src_layer = (LayerPtr)Hash_Get_Value(layer_hash,
                                                     (unsigned long)src);
    LayerPtr    dest_layer = (LayerPtr)Hash_Get_Value(layer_hash,
                                                      (unsigned long)dest);
    int         temp;
    int         i;

    if ( ( src == 0 && dest == 1 ) || ( dest == 0 && src == 1 ) )
    {
        XBell(XtDisplay(layer_list_widget), 0);
        return;
    }
...
Layer_Set_Instance_Visibility(LayerPtr layer)
{
    ...
    for ( i = 0 ; i < layer->num_instances ; i++ )
        ...
        {
            layer->instances[i]->o_flags &=
                (ObjAll ^ ( ObjVisible | ObjSelected ) );
...
Layer_Toggle_Callback(Widget w, XtPointer cl, XtPointer ca)
{
    ...
    layer = (LayerPtr)Hash_Get_Value(layer_hash, (unsigned long)layer_num);
...
Add_New_Layer(int num, char *name, Boolean state)
{
    ...
    Hash_Insert(layer_hash, (unsigned long)num, (void*)new_layer);
...
New_Layer_Callback(Widget w, XtPointer cl, XtPointer ca)
{
    ...
    Position_Shell(new_layer_shell, FALSE);
...
Add_Layer_Callback(Widget w, XtPointer cl, XtPointer ca)
{
    ...
    Position_Shell(layer_list_shell, FALSE);
...
Visible_Layer_Callback(Widget w, XtPointer cl, XtPointer ca)
{
    ...
    Position_Shell(layer_toggle_shell, FALSE);
...
Layers_Init()
{
    ...
    Add_New_Layer(1, "Construction", TRUE);
}
...
{
    if ( ! layer )
        layer = (LayerPtr)Hash_Get_Value(layer_hash, (unsigned long)num);
